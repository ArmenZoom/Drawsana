...
public class DrawsanaView: UIView {
    ...
    public var panGR: UIPanGestureRecognizer? = nil
    public var tapGR: UITapGestureRecognizer? = nil

    
    public var scrollViewPinchGesture: UIPinchGestureRecognizer?
    public var scrollViewPanGesture: UIPanGestureRecognizer?
     ...
     */
    public var transientBufferWithShapeInProgress: UIImage?
    ...
    public func clean() {
        ...
        let strokeWidth = self.userSettings.strokeWidth
        self.userSettings.strokeWidth = 3000
        ...
        tool.handleDragStart(context: self.toolOperationContext, point: point)
        updateUncommittedShapeBuffers()
        tool.handleDragEnd(context: self.toolOperationContext, point: point)
        self.delegate?.drawsanaView(self, didEndDragWith: tool)
        self.reapplyLayerContents()
        
        self.applyToolSettingsChanges()
        self.userSettings.strokeWidth = strokeWidth
    }
    
    public func addImage(image: UIImage, rect: CGRect) {
        self.drawing.size = bounds.size

        let shape = ImageShape(image: image, rect: rect)
        shape.apply(userSettings: self.toolOperationContext.userSettings)
        self.toolOperationContext.operationStack.apply(operation: AddShapeOperation(shape: shape, stepCount: self.stepCount))
    ...
    public func resetGesture() {
        if let panGR = self.panGR { self.removeGestureRecognizer(panGR) }
        if let tapGR = self.tapGR { self.removeGestureRecognizer(tapGR) }
        
        let panGR = UIPanGestureRecognizer(target: self, action: #selector(didPan(sender:)))
...
//        }
    ...
    @objc private func didTap(sender: UITapGestureRecognizer) {
        autoreleasepool { _didTap(sender: sender) }
    }
    
    private func _didTap(sender: UITapGestureRecognizer) {
        guard let tool = tool else { return }
        ...
        tool.handleDragStart(context: self.toolOperationContext, point: point)
        self.delegate?.drawsanaView(self, didStartDragWith: tool)
        updateUncommittedShapeBuffers()
        tool.handleDragEnd(context: self.toolOperationContext, point: point)
        self.delegate?.drawsanaView(self, didEndDragWith: tool)
        self.reapplyLayerContents()
    ...
    private func _didPan(sender: UIPanGestureRecognizer) {
        ...
        case .began:
            if let persistentBuffer = persistentBuffer, let cgImage = persistentBuffer.cgImage {
                transientBuffer = UIImage(cgImage: cgImage,
            ...
            updateUncommittedShapeBuffers()
        case .changed:
            tool.handleDragContinue(context: toolOperationContext, point: point, velocity: .zero)
            updateUncommittedShapeBuffers()
            delegate?.drawsanaView(self, didDragWith: tool, point: point)
        case .ended:
...
extension DrawsanaView: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        guard let pinchGesture = self.scrollViewPinchGesture,
            let panGesture = self.scrollViewPanGesture,
            let drawPan = self.panGR,
            drawPan == gestureRecognizer else {
                return true
        }
