...
public class DrawingOperationStack {
    ...
    public var redoStack = [DrawingOperation]()
    ...
    @objc public func undo(step: Int = 1) {
        ...
        if let operation = operation as? AddShapeOperation,
           operation.stepCount > step {
            self.undo(step: step + 1)
        }
    }
    
    /// Redo the most recently undone operation, if any
    @objc public func redo(step: Int = 1) {
